...
export class AiRealService {
  ...
  async smartPrice(body: any) {
    const result = await this.runJsonTask('pricing', body, [
      'Return JSON with category, suggestedPrice, minPrice, maxPrice, confidence and explanation.',
      'Prices must be BRL numbers for a Brazilian local services marketplace.',
    ]);
    const suggestedPrice = this.roundCurrency(
      this.readNumber(result.data.suggestedPrice, this.localPriceFallback(body)),
    );
    const commissionRate = this.readNumber(body?.commissionRate, 0.1);
    const commission = this.roundCurrency(suggestedPrice * commissionRate);

    return {
      success: true,
      provider: result.provider,
      fallbackUsed: result.fallbackUsed,
      category: this.readString(result.data.category) ?? body?.category ?? 'geral',
      suggestedPrice,
      minPrice: this.roundCurrency(
        this.readNumber(result.data.minPrice, suggestedPrice * 0.85),
      ),
      maxPrice: this.roundCurrency(
        this.readNumber(result.data.maxPrice, suggestedPrice * 1.25),
      ),
      confidence: this.readNumber(result.data.confidence, 0),
      commission,
      professionalReceives: this.roundCurrency(suggestedPrice - commission),
      explanation: this.readString(result.data.explanation),
      timestamp: new Date().toISOString(),
    };
  }
  ...
  private async runJsonTask(
    ...
    for (const provider of providers) {
      try {
        const data = await this.callProvider(provider, task, payload, instructions);
  ...
  private availableProviders(): AiProvider[] {
    ...
    const preferred =
      preference === 'gemini' || preference === 'openai' ? preference : undefined;
