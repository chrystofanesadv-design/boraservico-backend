...
export class ReferralService {
  ...
  async calculateBonus(data: any) {
    ...
    return this.prisma.$transaction(async (tx: any) => {
      ...
      const now = new Date();
      const referral = await this.refreshReferralPhase(
        tx,
        activeReferral,
        now,
      );
      ...
      if (duplicateBonus) {
        ...
        await this.auditService.register('REFERRAL_DUPLICATE_BONUS_BLOCKED', {
          ...
          entityType: 'referralBonus',
          entityId: duplicateBonus.id,
          orderId: sourceEventId,
          metadata: {
            referralId: referral.id,
  ...
  private async refreshReferralList(
    ...
    for (const referral of referrals) {
      refreshed.push(await this.refreshReferralPhase(this.prisma, referral, now));
    }
  ...
  private buildShareContract(userId: string) {
    ...
    const whatsappMessage =
      `Bora comigo no BoraServico? Use meu codigo ${referralCode} ` +
