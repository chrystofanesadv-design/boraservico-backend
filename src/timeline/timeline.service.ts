...
export class TimelineService {
  ...
  async createEvent(data: any) {
    ...
    const event: TimelineEvent = {
      ...
      subtitle: this.readString(data?.subtitle) || this.subtitleForType(type),
      description: this.readString(data?.description) || this.subtitleForType(type),
  ...
  checkOut(data: any) {
    return this.createEvent({
      ...
      type: 'CHECKED_OUT',
      title: 'Check-out',
      description: data?.description ?? 'Servico finalizado no local com evidencia.',
      latitude: data?.latitude ?? data?.lat,
      longitude: data?.longitude ?? data?.lng,
    ...
    try {
      return await this.prisma.operationalTimelineEvent.create({
        data: {
          ...
          title: this.readString(data?.title) || this.titleForType(type),
          description: this.readString(data?.description) || this.subtitleForType(type),
          state: this.toPrismaState(this.readState(data?.state) || 'current') as any,
          timestamp: new Date(),
  ...
  private defaultTimeline(orderId: string) {
    ...
    return [
      ...
      this.defaultEntry(orderId, 'match', 'Profissional encontrado', 'current', now),
  ...
  private toPublicPrismaEvent(event: any) {
    ...
    const subtitle = this.readString(metadata.subtitle) || this.subtitleForPhase(phase);
  ...
  private phaseForType(type: TimelineEventType) {
    const phases: Record<TimelineEventType, string> = {
      ...
      PROFESSIONAL_NEARBY: 'route',
      CHECKED_IN: 'checkIn',
      IN_PROGRESS: 'execution',
      CHECKED_OUT: 'proof',
      SERVICE_FINISHED: 'proof',
  ...
  private titleForType(type: TimelineEventType) {
    const titles: Record<TimelineEventType, string> = {
      ...
      PROFESSIONAL_NEARBY: 'Profissional proximo',
      CHECKED_IN: 'Check-in',
  ...
  private readState(value: any) {
    ...
    if (state === 'complete' || state === 'current' || state === 'upcoming' || state === 'alert') {
      return state;
    }
